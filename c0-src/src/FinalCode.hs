...
pogTranslate (COND t1 relop t2 label1 label2) =
                         (relopTranslate relop) ++ "$" ++ t1 ++ ", " ++ "$" ++ t2 ++ ", " ++ label1 ++ "\n" ++ pogTranslate (JUMP label2)
...
-- restoreRegsAux :: [Temp] -> Int -> String
-- restoreRegs i temps =
--
-- restoreReg :: Int -> Temp -> String
-- restoreReg i temp  =
--
